...
kali 攻击机 ip：192.168.30.130

靶机 ip：192.168.30.136

注：靶机若为仅主机模式，后面无法反弹 shell
...
![image-20221213172923933.png](https://img.ghostliner.top/K7zhSS.png)

```shell
nmap -A 192.168.30.136
```

![image-20221212162322876.png](https://img.ghostliner.top/qwsEbu.png)

DNS:earth.local, DNS:terratest.earth.local

发现 443 端口有 DNS 解析，在 hosts 文件中添加 DNS 解析

![image-20221213173028858.png](https://img.ghostliner.top/0wm6k5.png)
...
![image-20221212163628334.png](https://img.ghostliner.top/LzpM2D.png)
...
**分析：**此处明文，对应 3 个密文中的一个。因为三个密文长度不一，明文大约 403 字符，第三个密文为 806 个字符。
...
`int(data1,16)`将字符串作为 16 进制转为整数

`binascii.b2a_hex()`将二进制数据转换为十六进制字符串表示

`hex()`将 10 进制整数转为 16 进制

对第三个密文进行解密，获取 16 进制数，转为字符串获取密钥：
...
在登录界面输入 username=terra,password=earthclimatechangebad4humans
...
![image-20221212173800473.png](https://img.ghostliner.top/fIPnMB.png)
...
![image-20221213171621009.png](https://img.ghostliner.top/uWBqnb.png)
...
![image-20221213171742390.png](https://img.ghostliner.top/VzwhKX.png)
...
![image-20221213184633800.png](https://img.ghostliner.top/CreORq.png)
...
![image-20221213185021050.png](https://img.ghostliner.top/7wBUBS.png)

因为没有这三个文件而报错，尝试手动创建，使用 touch

![image-20221213185318191.png](https://img.ghostliner.top/ktQKnU.png)

![image-20221213185353158.png](https://img.ghostliner.top/l4vPyi.png)

### 8. 获取 rootflag

尝试使用密码 Earth 登录 root 账户成功
...
![image-20221213193300205.png](https://img.ghostliner.top/9SwHUM.png)
