...
#### 配置邮件通知
配置目录`/user/acme/example`下的`account.conf`，添加配置：
```
...
使用acme.sh命令即可完成上述操作，将证书更新和安装合并。

创建脚本：
```shell
#!/bin/bash
# update *.v6.army cert to WAF
host=""
# 设计host字典
declare -A dict=(
	["example1"]=1
...
# 执行更新证书命令
output=$(/home/load/.acme.sh/acme.sh --renew-all --config-home "/root/acme/${host}")
# 检查是否更新
...
# 安装证书到指定路径
/home/load/.acme.sh/acme.sh --config-home "/root/acme/${host}" \
